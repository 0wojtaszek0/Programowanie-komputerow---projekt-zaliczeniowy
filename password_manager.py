...
class PasswordManagerApp(ctk.CTk):
    ...
    def _fill_generated_password(self, password_entry, strength_bar, length_var, lowercase_var, uppercase_var, digits_var, special_var, strength_label=None):
        chars = ""
        required = []
    ...
    def _open_password_dialog(self, entry=None):
        ...
        for text, var in (("Małe litery", lowercase_var), ("Wielkie litery", uppercase_var), ("Cyfry", digits_var), ("Znaki specjalne", special_var)):
            box = ctk.CTkCheckBox(generator_frame, text=text, variable=var)
            box.pack(anchor="w", padx=16)
            self._apply_font(box)
        ctk.CTkButton(generator_frame, text="Wygeneruj hasło", command=lambda: self._fill_generated_password(password_entry, strength_bar, length_var, lowercase_var, uppercase_var, digits_var, special_var, strength_label)).pack(pady=10)
